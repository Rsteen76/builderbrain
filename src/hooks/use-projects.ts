...
export function useProjects(userId: string, filters?: ProjectFilters) {
  return useQuery(
    [PROJECTS_QUERY_KEY, userId, filters],
    async () => {
  ...
  return useMutation(
    ...
    },
    {
      onSuccess: (newProject: Project) => {
        ...
        queryClient.invalidateQueries([PROJECTS_QUERY_KEY, newProject.userId]);
  ...
  return useMutation(
    ...
    },
    {
      onSuccess: (updatedProject: Project) => {
        ...
        // Invalidate the projects list query to refetch
        queryClient.invalidateQueries([PROJECTS_QUERY_KEY, updatedProject.userId]);
      },
    }
  );
}
  ...
  return useMutation(
    ...
    },
    {
      onSuccess: ({ id, userId }: { id: string; userId: string }) => {
        ...
        queryClient.removeQueries([PROJECTS_QUERY_KEY, id]);
        
        // Invalidate the projects list query to refetch
        queryClient.invalidateQueries([PROJECTS_QUERY_KEY, userId]);
      },
    }
  );
} 