...
const mockRenderExpenseRow = jest.fn();
...
describe('ExpenseTable', () => {
  beforeEach(() => {
    mockRenderExpenseRow.mockImplementation((expense: Expense) => (
      <TableRow key={expense.id}>
        <TableCell>{mockExpenseRowContent}</TableCell>
  ...
  test('renders table with data and grouping by project', () => {
    ...
    expect(screen.getAllByText('$1,350.00').length).toBeGreaterThan(0); // Group total in header
    ...
    expect(screen.getAllByText('$500.00').length).toBeGreaterThan(0); // Group total in header
  ...
  test('displays sort direction indicator for Amount', () => {
    const { rerender } = renderWithTheme(<ExpenseTable {...defaultProps} sortField="amount" sortDirection="asc" />);
    ...
    rerender(
      <ThemeProvider theme={mockTheme}>
        <ExpenseTable {...defaultProps} sortField="amount" sortDirection="desc" />
      </ThemeProvider>
    );
    expect(within(screen.getByText('Amount').closest('th')!).getByTestId('ArrowDropDownIcon')).toBeInTheDocument();
  });

  test('displays sort direction indicator for Date', () => {
    const { rerender } = renderWithTheme(<ExpenseTable {...defaultProps} sortField="date" sortDirection="asc" />);
    const dateHeader = screen.getByText('Date').closest('th');
    expect(within(dateHeader!).getByTestId('ArrowDropUpIcon')).toBeInTheDocument();

    rerender(
      <ThemeProvider theme={mockTheme}>
        <ExpenseTable {...defaultProps} sortField="date" sortDirection="desc" />
      </ThemeProvider>
    );
    expect(within(screen.getByText('Date').closest('th')!).getByTestId('ArrowDropDownIcon')).toBeInTheDocument();
  });
