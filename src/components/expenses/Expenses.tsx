...
import {
  ...
  // TableRow, // Removed, used in ExpenseTable
  // TableCell, // Removed, used in ExpenseTable
...
import { Expense, Project, ProjectPhase, ExpenseCategory, ExpenseStatus, PaymentDetails as ExpensePaymentDetails } from '../../types';
import { useAuth } from '../../contexts/AuthContext';
import { useGetExpenses, UseExpensesFilters } from '../../hooks/use-expenses'; // Import the new hook
import { formatCurrency, formatDate } from '../../utils/formatters';
import ExpenseFormModal from './ExpenseFormModal';
import PaymentFormModal from './PaymentFormModal';
import { BidService, adjustBidPaymentSchedule } from '../../services/bid'; // Modified import
import { BidPaymentStage } from '../../types';
// import { v4 as uuidv4 } from 'uuid'; // uuidv4 is used in adjustBidPaymentSchedule, now in bid.ts
...
const Expenses: React.FC<{ projectId?: string }> = ({ projectId }) => {
  ...
  // Define a more specific type for raw phase data if possible, or use Partial<ProjectPhase> if structure aligns
  interface RawPhaseData { // Local type for un-processed phase data
    id?: string;
    name?: string;
    startDate?: string | Date | import('firebase/firestore').Timestamp;
    endDate?: string | Date | import('firebase/firestore').Timestamp;
    status?: string; 
    progress?: number;
    budget?: number;
    actualCost?: number;
    description?: string;
    tasks?: unknown[]; // Can be refined if task structure is known and consistent
    order?: number;
  }

  const mapToProjectPhase = (phaseData: RawPhaseData): ProjectPhase => {
    const id = phaseData.id ?? ''; // Provide default if id can be undefined from source
    const name = phaseData.name ?? 'Unnamed Phase'; // Provide default
    ...
    if (selectedExpenseId && user?.uid) {
      ...
      ExpenseService.deleteExpense(selectedExpenseId) // This still uses ExpenseService directly, consider useDeleteExpense hook later
        ...
        .catch((err: unknown) => {
          ...
          setError('Failed to delete expense. Please try again. ' + errorMessage);
          setSnackbar({
            open: true,
  ...
  const handleMarkAsPaid = async (expenseId: string, actualAmountPaid: number, paymentDetails?: ExpensePaymentDetails) => {
    if (!user?.uid) return;
    
    // console.log(`[handleMarkAsPaid START] ID: ${expenseId}, Amount Paid Now: ${actualAmountPaid}`, paymentDetails); // Keep if needed
    setSubmitting(true);
    ...
    try {
      ...
      setError(message || 'Failed to mark expense as paid. Please try again.');
      setSnackbar({
        open: true,
