...
jest.mock('firebase/firestore', () => {
  ...
  return {
    addDoc: jest.fn(),
    collection: jest.fn((_db, path) => `mocked_collection_path_for_${path}`),
    deleteDoc: jest.fn(),
    doc: jest.fn((_collectionRef, id) => `mocked_doc_ref_${id}`),
    getDoc: jest.fn(),
    getDocs: jest.fn(),
    limit: jest.fn(),
    orderBy: jest.fn(),
...
import { Timestamp, addDoc, collection, doc, getDoc, updateDoc } from 'firebase/firestore'; // For mocking Firestore Timestamp

const installTimestampMocks = () => {
  (Timestamp.fromDate as jest.Mock).mockImplementation((date: Date) => new (Timestamp as any)(date));
  (Timestamp.now as jest.Mock).mockImplementation(() => new (Timestamp as any)(new Date()));
};
...
describe('BidService - adjustBidPaymentSchedule', () => {
  ...
  beforeEach(() => {
    ...
    installTimestampMocks();
    mockGetBid = jest.spyOn(BidService, 'getBid').mockResolvedValue(null) as jest.Mock;
    mockUpdateBid = jest.spyOn(BidService, 'updateBid').mockResolvedValue(undefined) as jest.Mock;
  });
  ...
  test('should fully pay a pending stage and update bid progress', async () => {
    ...
    expect(updatedBidData.paymentProgress.pending).toBe(0);
  });
...
describe('BidService - createBid / updateBid', () => {
  ...
  describe('updateBid', () => {
    test('should call updateDoc with transformed partial bid data', async () => {
      ...
      expect(mockDoc).toHaveBeenCalledWith('mocked_collection_path_for_bids', bidId);
