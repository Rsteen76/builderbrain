...
- Phase 4 partially complete in this branch:
  - Completed: dependency vulnerabilities, auth startup failure handling, payment save reliability, data reset reliability, partial expense update safety, shared report ID entropy, accepted-bid expense idempotency, storage validation, hosting headers/CSP, payments N+1 query, dashboard summary read model, project-detail data boundary, Calendar/Timeline data boundary, budget write boundary, Settings stub cleanup, Vite migration, React Query v5 migration, service/page module splits, budget dashboard split, production chunk cleanup, docs/test gates.
  - Not fully closed: some UI modules still exceed the target file-size ceiling and direct console usage remains in lower-priority surfaces.
...
12. Large god modules and page components block safe change.
    ...
    - Fix: split project, bid, dev data store, budget dashboard, expenses page, and expense form concerns into cohesive helper/component modules with focused tests. Continue shrinking remaining oversized UI surfaces in later focused PRs.
...
- Continue shrinking oversized UI surfaces along cohesive boundaries; `bid.ts`, `project.ts`, `devDataStore.ts`, `BudgetDashboard.tsx`, `Expenses.tsx`, and `ExpenseFormModal.tsx` now have extracted helper/component boundaries.
